    ...
    <div className="relative z-20 flex w-full flex-col gap-4">
      {feedback.map(([label, message, reply], index) => (
        ...
        >
          ...
          <div className="mr-6 rounded-2xl rounded-tl-md border border-slate-200 bg-white px-3 py-2.5 text-sm leading-5 text-slate-900 shadow-sm">
            <p>{message}</p>
            <span className="mt-1 block text-right text-[11px] text-slate-400">03:19</span>
          </div>
          <div className="ml-6 rounded-2xl rounded-tr-md border border-black/[0.06] bg-[#DCF8C6] px-3 py-2.5 text-sm leading-5 text-[#111827] shadow-sm">
            <p>{reply}</p>
