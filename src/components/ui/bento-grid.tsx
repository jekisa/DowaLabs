...
export function BentoGrid({ items, ctaLabel, className }: BentoGridProps) {
  return (
    <div className={cn("grid grid-cols-1 gap-3 md:grid-cols-6 md:gap-4", className)}>
      {items.map((item) => (
...
function BentoCard({ item, ctaLabel }: { item: BentoCardItem; ctaLabel: string }) {
  ...
  return (
    <motion.article
      ...
      className={cn("group relative flex min-h-0 flex-col overflow-hidden md:min-h-[400px] rounded-2xl border border-slate-200 bg-white shadow-[0_8px_30px_rgba(15,23,42,0.05)] transition duration-300 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-[0_12px_34px_rgba(15,23,42,0.08)]", item.className)}
    >
      ...
      <div className="relative h-[150px] shrink-0 overflow-hidden md:h-auto md:flex-1">
        ...
        <div className="relative z-10 flex items-start justify-between gap-4 p-4 md:p-5">
      ...
      <div className="relative bg-white px-4 pb-4 pt-3 md:px-5 md:pb-5 md:pt-4">
        <h3 className="text-xl font-semibold text-slate-900">{item.title}</h3>
        <p className="mt-2 max-w-lg text-sm leading-6 text-slate-600 md:mt-3">{item.description}</p>
        <Link href={item.href || "/signup"} className="mt-3 inline-flex items-center gap-2 text-sm font-medium md:mt-4 text-slate-900 transition duration-300 hover:text-amber-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-300">
          {ctaLabel}
